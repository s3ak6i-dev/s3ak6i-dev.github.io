  ...
  useEffect(() => {
    ...
    const distortions = {
      mountainDistortion: {
        ...
        getJS: (progress, time) => {
          let f = 0.02, uFreq = mountainUniforms.uFreq.value, uAmp = mountainUniforms.uAmp.value;
      ...
      xyDistortion: {
        ...
        getJS: (progress, time) => {
          let f = 0.02, uFreq = xyUniforms.uFreq.value, uAmp = xyUniforms.uAmp.value;
          let d = new THREE.Vector3(
            Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(f * Math.PI * uFreq.x + time) * uAmp.x,
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
          let c = 0.0125, uFreq = LongRaceUniforms.uFreq.value, uAmp = LongRaceUniforms.uAmp.value;
          let d = new THREE.Vector3(
      ...
      turbulentDistortionStill: {
        ...
        getDistortion: `
          ...
          float nsin(float val){ return sin(val)*0.5+0.5; }
          #define PI 3.14159265358979
          float getDistortionX(float progress){ return cos(PI*progress*uFreq.r)*uAmp.r + pow(cos(PI*progress*uFreq.g*(uFreq.g/uFreq.r)),2.)*uAmp.g; }
          float getDistortionY(float progress){ return -nsin(PI*progress*uFreq.b)*uAmp.b + -pow(nsin(PI*progress*uFreq.a/(uFreq.b/uFreq.a)),5.)*uAmp.a; }
    ...
    const roadMarkings_fragment = `
      ...
      float laneWidth=1.0/uLanes, brokenLineWidth=laneWidth*uBrokenLinesWidthPercentage, laneEmptySpace=1.-uBrokenLinesLengthPercentage;
    ...
    options.distortion = distortions[options.distortion] || { uniforms: distortion_uniforms, getDistortion: distortion_vertex };
